...
int download_package_files(const char *pkg_url, const char *sig_url, const char *sha256_url, const char *pkg_path, const char *sig_path, const char *sha256_path) {
...
int repman_download_and_install_pkg(const char *name, const char *version, const char *os, const char *arch) {
    ...
    if (strcmp(name, "repman") == 0) {
        fprintf(stderr, "error: use 'repman upgrade' to update repman itself\n");
        return -1;
    }
...
/* unlink_pkg_data, repman_uninstall moved to uninstall.c */
/* repman_self_update moved to selfupdate.c               */